...
export default function PedidosPage() {
  ...
  function handleAdicionarAoCarrinho() {
    ...
    setCarrinho(atual => {
      const existente = atual.find(item => item.variacao.id === variacao.id);
      if (!existente) return [...atual, { variacao, quantidade: quantidadeNumerica }];
      return atual.map(item => item.variacao.id === variacao.id
        ? { ...item, quantidade: item.quantidade + quantidadeNumerica }
        : item);
    });
    setQuantidade('1');
  }
